...
class RealTimeOrderbookTest {
    ...
    void testGetAskOrderbook() {
        ...
        assertEquals(2.0, result.get(0).size());
        assertEquals(200.0, result.get(1).price());
        assertEquals(3.0, result.get(1).size());
    }
    ...
    void testGetAskOrderbookEmpty() {
        ...
        assertThrows(IllegalArgumentException.class, () -> orderbook.getAskOrderbook(code));
    }
    ...
    void testGetBidOrderbook() {
        ...
        assertEquals(2, result.size());
        assertEquals(300.0, result.get(0).price());
        assertEquals(5.0, result.get(0).size());
        assertEquals(400.0, result.get(1).price());
        assertEquals(4.0, result.get(1).size());
    }
    ...
    void testGetBidOrderbookEmpty() {
        ...
        Mockito.when(zSetOperations.reverseRangeWithScores("orderbook:" + code + ":bids", 0, -1))
               .thenReturn(Collections.emptySet());
        assertThrows(IllegalArgumentException.class, () -> orderbook.getBidOrderbook(code));
    }
}
